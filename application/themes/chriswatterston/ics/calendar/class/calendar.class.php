...
class Calendar {
	...
	function __construct($settings = array()) {
		...
		if(!$this->_get_cache()) {
			if(!$this->_retrieve_data()) {
				echo '<p class="alert-warning">We have failed to retrieve data from Saint Michael Primary School calendar ICS. Please visit again soon and try again.<p>';
				$this->error = true;
				return false;
			}
			if(!$this->_parse_data()) {
				echo '<p class="alert-warning">We have failed to parse data from Saint Michael Primary School calendar ICS. Please visit again soon and try again.<p>';
				$this->error = true;
				return false;
			}
	...
	public function output() {
		...
		for($i = 0; $i < count($events); $i++) {
			...
			// remove past events
			$entPassed = date('j-n-Y', $event['start']);
			$tDate = date('j-n-Y');
			
			$entPassed = strtotime($entPassed);
			$tDate = strtotime($tDate . ' -1 days');
									
			if($entPassed > $tDate) {
				$entCount++;
			
				if($entCount==6) break; // STMICHAEL Limit
			    ...
			    if ($days_remaining > 1){
				  $days_remaining_result = "<p class='snip colour-black'>Few days ago</p>";
				} elseif ($days_remaining == 1){
					$days_remaining_result = "<p class='snip colour-black'>Ago</p><p class='days colour-black'>1</p>";
				} elseif ($days_remaining == 0){
					$days_remaining_result = "<p class='snip colour-black'>Today</p>";
				} elseif ($days_remaining == -1){
					$days_remaining_result = "<p class='snip colour-black'>Day</p><p class='days colour-black'>1</p>";
				} elseif ($days_remaining < -1){
					$days_remaining_fltrd = str_replace("-","",$days_remaining);
					$days_remaining_result = "<p class='snip colour-black'>Days</p><p class='days colour-black'>$days_remaining_fltrd</p>";
				}
	
				$html .= '<div class="cal-event-day">'. $days_remaining_result .'</div>';
				...
				if($event['start'] + 86400 == $event['end'] && date('G', $event['start']) == abs($this->settings['extra_time']/3600*$xml_extra_time)) {
					...
					$html .= '<div class="cal-event-date"><strong>'.date('jS F Y'.$date_day_addition, $event['start']).'</strong></div>';
	
				}elseif($event['start'] + 86400 >= $event['end'] && date('G', $event['start']) < date('G', $event['end'])) {
					//event on one day
					$html .= '<div class="cal-event-date"><strong>'.date('jS F Y'.$date_day_addition.' @ '.$time_format, $event['start']).'</strong></div>';
				...
				//echo '<p class="alert-warning">No Events</p>';
			}
		}
