...
<?php  if (in_array($controller->getTask(), $listViews)){ ?>
    ...
    <div class="ccm-dashboard-content-full">
        <table class="ccm-search-results-table">
            <thead>
                <th><a><?php echo t('Name')?></a></th>
                <th><a><?php echo t('Display')?></a></th>
                <th><a><?php echo t('Discount')?></a></th>
                <th><a><?php echo t('Applies')?></a></th>
                <th><a><?php echo t('Availability')?></a></th>
                <th><a><?php echo t('Enabled')?></a></th>
            ...
            <tbody>
                ...
                <?php  if(count($discounts)>0) {
                    foreach ($discounts as $d) {
                        ?>
                        <tr>
                            <td><strong><a href="<?php  echo View::url('/dashboard/store/discounts/edit/', $d->drID)?>"><?php echo  h($d->drName); ?></a></strong></td>
                            <td><?php echo  h($d->drDisplay); ?></td>
                            <td>
                            ...
                            <td><span class="label label-<?php echo  ($d->drEnabled ? 'success">' . t('Enabled') : 'danger">' . t('Disabled')); ?></span></td>
                            <td>
...
<?php  if (in_array($controller->getTask(), $addViews)){ ?>
    ...
    <?php  if ($controller->getTask() == 'edit') { ?>
        <div class="ccm-dashboard-header-buttons">
            <form method="post" id="deleterule" action="<?php  echo View::url('/dashboard/store/discounts/delete/')?>">
                <input type="hidden" name="drID" value="<?php echo  $d->drID; ?>" />
    ...
    <div class="ccm-pane-body">
        ...
        <div id="codefields" <?php  echo ($d->drTrigger == 'auto' ? 'style="display: none;"' : ''); ?>>
            ...
            <?php  if (!$d->drID) { ?>
            <p class="alert alert-info"><?php echo  t('Codes can be entered after creating rule');?></p>
            <?php  } ?>
        </div>

<!--       <field name="drCurrency" type="C" size="20"></field>-->

        <fieldset><legend><?php echo  t('Restrictions');?></legend>

        <div class="form-group">
            <?php  echo $form->checkbox('drExclusive', '1', $d->drExclusive)?>
            <?php  echo $form->label('drExclusive', t('Discount is Exclusive'))?>
            <span class="help-block"><?php echo  t('When checked, discount cannot be used in conjunction with another discount'); ?></span>
        </div>
    ...
    <script>
        $(function(){
            $('#deleterule').submit(function(e){
                return confirm("<?php echo  t('Are you sure you want to delete this discount rule?');?>");
            });
...
<p class="alert alert-success"><?php echo  $successCount . ' ' . ($successCount == 1 ? t('code added') : t('codes added')); ?></p>
<?php  } ?>

<?php  if (isset($failedcodes) && count($failedcodes) > 0 ) { ?>
    <p class="alert alert-warning"><?php echo  t('The following codes are were invalid or are already active:')  ?><br />
        <strong><?php echo  implode('<br />', $failedcodes); ?></strong>
    </p>
<?php  } ?>



<fieldset><legend><?php echo  t('Current Codes'); ?></legend></fieldset>

<p class="alert alert-info">
    <?php  if ($d->drSingleUseCodes) { ?>
    <?php echo  t('Single Use Codes'); ?></p>
<?php  } else { ?>
    <?php echo  t('Codes can be repeatedly used'); ?>
<?php  } ?>
</p>
...
<?php  if (!empty($codes)) { ?>
        <table class="table table-bordered">
            ...
            <?php  foreach($codes as $code) { ?>
                    <?php  if ($d->drSingleUseCodes) { ?>
                        ...
                        <?php  if ($code['oDate']) { ?>
                            <tr>
                                <td><strike><?php echo  $code['dcCode']; ?></strike></td>
                                <td><?php  echo $code['oDate']; ?> <a class="btn btn-default btn-xs" href="<?php echo  View::url('/dashboard/store/orders/order/', $code['oID']); ?>"><?php echo  t('View Order'); ?></a></td>
                                <td></td>
                            </tr>
                        <?php  } else { ?>
                            <tr>
                                <td><?php echo  $code['dcCode']; ?></td>
                                <td><span class="label label-success"><?php echo  t('Available'); ?></span></td>
                            <td>
    ...
    <?php  } else { ?>
    <p><?php echo  t('No codes specified');?></p>

    <?php  } ?>
<br />
<form method="post" action="<?php  echo View::url('/dashboard/store/discounts/addcodes', $d->drID)?>" id="codes-add">
<fieldset><legend><?php echo  t('Add Codes'); ?></legend>

    <div class="form-group">
        <?php  echo $form->label('codes', t('Code(s)'))?>
        <?php  echo $form->textarea('codes', '', array('class' => ''))?>
        <span class="help-block"><?php echo  t('Seperate codes via lines or commas. Codes are case-insensitive.'); ?></span>
    </div>

    <div class="form-group">
        <button type="submit" class="btn btn-success"><?php echo  t('Add Codes'); ?></button>
    </div>
